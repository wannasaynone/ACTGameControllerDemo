...
public class Controller : MonoBehaviour {
    ...
    private void AICondition()
    {
        if(aiConditionJudgeTimer <= 0)
        {
            ...
            if (GameManager.insance.GetCharacterState(GameManager.insance.GetPlayerGuid()).CurrentHP < 0)
                _currnetAIState = aiMotionOptions.Idle;
            else
            if (GameManager.insance.GetCharacterState(GameManager.insance.GetPlayerGuid()).isInvincible)
            {
                if(GameRoot.PLAYER.transform.position.x > transform.position.x)
            ...
            if (GameRoot.PLAYER.transform.position.x > transform.position.x && Mathf.Abs(GameRoot.PLAYER.transform.position.x - transform.position.x) > 2f)
                _currnetAIState = aiMotionOptions.Run_Right;
            else
            if (GameRoot.PLAYER.transform.position.x < transform.position.x && Mathf.Abs(GameRoot.PLAYER.transform.position.x - transform.position.x) > 2f)
                _currnetAIState = aiMotionOptions.Run_Left;
            else
            if (GameRoot.PLAYER.transform.position.x > transform.position.x && Mathf.Abs(GameRoot.PLAYER.transform.position.x - transform.position.x) > 0.5f)
                _currnetAIState = aiMotionOptions.Move_Right;
            else
            if (GameRoot.PLAYER.transform.position.x < transform.position.x && Mathf.Abs(GameRoot.PLAYER.transform.position.x - transform.position.x) > 0.5f)
                _currnetAIState = aiMotionOptions.Move_Left;
